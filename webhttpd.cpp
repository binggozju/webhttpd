...
// 存储master进程用来与各worker进程通信的unix域套接字
vector<int> child_fds;
// master进程将向child_fds[child_index]写入新的连接套接字，传递给对应的worker子进程 
int child_index;
// 在worker子进程中存储的是用来与master进程通信的unix域套接字，在master进程中为-1
int parent_fd;
...
char uri_root[512];
content_type_t content_type_table[] = {
	{"txt",		"text/plain"},
	{"html",	"text/plain"},
	{"htm",		"text/plain"},
	{"gif",		"image/gif"},
...
string guess_content_type(char *path)
{
	char *last_period, *extension;
	struct content_type_t *ent;
	...
	for(ent = &content_type_table[0]; ent->extension != ""; ++ent) {
		if (!evutil_ascii_strcasecmp(ent->extension.c_str(), extension))
			return ent->type;
	}
...
void send_document_cb(struct evhttp_request *req, void *arg)
{
	...
	if (S_ISDIR(st.st_mode)) {	// 如果用户请求的是一个目录
		 ...
		 * sent via sendfile */
		const char *type = guess_content_type(decoded_path).c_str();
